...
describe('AnalyticsService', () => {
  ...
  describe('getMetrics', () => {
    ...
    it('should return metrics successfully', async (): Promise<void> => {
      const result: IAnalyticsMetrics = await service.getMetrics();
      ...
      expect(UserModel.countDocuments).toHaveBeenCalledWith({ isActive: true });
    ...
    it('should handle database errors', async (): Promise<void> => {
      ...
      await expect(service.getMetrics()).rejects.toThrow(AppError);
  ...
  describe('getTrends', () => {
    ...
    it('should return trends with default time period', async (): Promise<void> => {
      const result: IAnalyticsTrends = await service.getTrends();
    ...
    it('should handle invalid time period', async (): Promise<void> => {
      await expect(service.getTrends('invalid')).rejects.toThrow(AppError);
    });
    ...
    it('should handle database errors', async (): Promise<void> => {
      ...
      await expect(service.getTrends()).rejects.toThrow(AppError);
  ...
  describe('getUserAnalytics', () => {
    ...
    it('should return user analytics successfully', async (): Promise<void> => {
      const result: IUserAnalytics = await service.getUserAnalytics(userId);
    ...
    it('should handle invalid userId', async (): Promise<void> => {
      await expect(service.getUserAnalytics('invalid-id'))
    ...
    it('should handle database errors', async (): Promise<void> => {
      ...
      await expect(service.getUserAnalytics(userId))
        .rejects.toThrow(AppError);
      expect(logger.error).toHaveBeenCalledWith('Failed to fetch user analytics', 
        expect.objectContaining({ error, userId: userId.toString() })
      );
    });
  });
}); 